...
\begin{future}
In the expression \chpl{var t:T = a;}, if \chpl{a} is of type \chpl{A} coercible
to type \chpl{T} and there exists a constructor
\chpl{T(a:A)} and also a copy constructor \chpl{T(t:T)}, then which is called?
\end{future}
...
In the current implementation, \chpl{_defaultOf(t:type)} is always called to
produce a value that initializes a variable or field of type \chpl{t} that is
declared without an explicit initializer.  The compiler-supplied version of
\chpl{_defaultOf(t:type)} calls the zero-argument constructor for type \chpl{t}.
It is possible to override the compiler-supplied version of \chpl{_defaultOf()}.
In that case, the user-defined version of \chpl{_defaultOf()} is used in
...
Thus, the user has two ways to override the behavior of \chpl{_defaultOf()}.
...
up in approximately the same place.  These paths are expected to diverge when \chpl{noinit}
is implemented.

The remainder of the code for these two cases first creates a domain expression and then
passes this to \chpl{chpl__buildArrayRuntimeType()}.  That function provides the
...
function performs the initialization as originally written in the \chpl{buildArrayRuntimeType}
function.
...
type-based initialization concludes with the \chpl{autoCopy} call.  The array literal is built by
...
The \chpl{autoCopy} to initialize the \chpl{type_tmp} is arguably correct.
...
The cases for \chpl{single} variables are symmetrical with sync variables, and the same comments
apply.
